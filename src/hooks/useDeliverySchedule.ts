...
import { convertTimeToMinutes } from "@/app/administrator/delivery-times/utils/convertTimeToMinutes";
import { Schedule } from "@/types/deliverySchedule";

/** Хук управления графиком доставки (админка) */
export function useDeliverySchedule() {
  ...
  const initializeEmptySchedule = useCallback(() => {
    ...
    dates.forEach((date) => { emptySchedule[date] = {}; });
  ...
  const fetchDeliveryTimes = useCallback(async () => {
    try {
      ...
      if (data.schedule && Object.keys(data.schedule).length > 0) {
        ...
        dates.forEach((date) => {
          updatedSchedule[date] = loadedSchedule[date] ? { ...loadedSchedule[date] } : {};
        });
  ...
  const updateTimeSlotStatus = useCallback(
    (date: string, timeSlot: string, free: boolean) => {
      setSchedule((prev) => ({ ...prev, [date]: { ...prev[date], [timeSlot]: free } }));
    }, []);
  ...
  const removeTimeSlot = useCallback((slotToRemove: string) => {
    ...
    const updatedSchedule: Schedule = { ...schedule };
    dates.forEach((date) => {
      if (updatedSchedule[date]) delete updatedSchedule[date][slotToRemove];
    });
    setSchedule(updatedSchedule);
  ...
  return {
    schedule, loading, saving, message, error, startTime, endTime, timeSlots,
    setStartTime, setEndTime, fetchDeliveryTimes, showMessage, addTimeSlot,
    updateTimeSlotStatus, removeTimeSlot, saveDeliveryTimes,
  };
}
