...
interface UsePricingProps {
  availableCartItems: Array<{ productId: string; quantity: number }>;
...
/** Хук расчёта цен корзины */
export const usePricing = ({
  availableCartItems, productsData, hasLoyaltyCard, bonusesCount, useBonuses,
}: UsePricingProps) => {
  const totalPrice = availableCartItems.reduce((total, item) => {
    ...
    const finalPrice = hasLoyaltyCard
      ? calculatePriceByCard(priceWithDiscount, CONFIG.CARD_DISCOUNT_PERCENT) : priceWithDiscount;
  ...
  const maxBonusUse = Math.min(bonusesCount, Math.floor((totalPrice * CONFIG.MAX_BONUSES_PERCENT) / 100));

  const finalPrice = useBonuses ? Math.max(0, totalPrice - maxBonusUse) : totalPrice;
  ...
  return { totalPrice, totalMaxPrice, totalDiscount, finalPrice, maxBonusUse, totalBonuses: totalBonuses(), isMinimumReached };
};
