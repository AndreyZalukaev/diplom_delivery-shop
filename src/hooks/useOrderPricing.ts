...
/** Хук расчёта цен для заказа (повторный заказ) */
export const useOrderPricing = (order: IOrder, currentProducts: CurrentProduct[]) => {
  const cartItemsForSummary: CustomCartItem[] = useMemo(
    () => order.items.map((item) => {
      const currentProduct = currentProducts.find((p) => p.id.toString() === item.productId.toString());
      if (!currentProduct) {
        return {
          productId: item.productId, quantity: item.quantity, price: item.price,
          discountPercent: item.discountPercent || 0,
          hasLoyaltyDiscount: item.hasLoyaltyDiscount || false, addedAt: new Date(),
        };
      }
      const priceAfterDiscount = currentProduct.basePrice * (1 - (currentProduct.discountPercent || 0) / 100);
      return {
        productId: item.productId, quantity: item.quantity, price: priceAfterDiscount,
        discountPercent: currentProduct.discountPercent || 0,
        hasLoyaltyDiscount: currentProduct.hasLoyaltyDiscount || false, addedAt: new Date(),
      };
    }),
  ...
  const customPricing: CustomPricing = useMemo(() => {
    ...
    const finalTotal = cartItemsForSummary.reduce((sum, item) => {
      const finalPrice = item.hasLoyaltyDiscount
        ? item.price * (1 - CONFIG.CARD_DISCOUNT_PERCENT / 100) : item.price;
    ...
    return {
      totalPrice: totalAfterProductDiscounts, totalMaxPrice: totalAfterProductDiscounts,
      totalDiscount, finalPrice: finalTotal, totalBonuses,
      maxBonusUse: 0, isMinimumReached: true,
    };
  }, [cartItemsForSummary]);
