...
import { formatDateFull, formatDateNumeric } from "@/app/administrator/delivery-times/utils/dateFormatters";
import { formatTimeSlot } from "@/app/(cart)/cart/utils/formatTimeSlot";
import { AvailableDate } from "@/types/availableDate";
import { getAvailableDates } from "@/utils/orders/getAvailableDates";
import { getAvailableTimeSlots } from "@/utils/orders/getAvailableTimeSlots";
  ...
  return (
    <div className="fixed inset-0 backdrop-blur-sm bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded max-w-md w-full mx-4">
        <h3 className="text-lg font-bold mb-4">Выберите дату и время доставки</h3>
        <div className="mb-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {availableDates.map((item) => {
              ...
              return (
                ...
                >
                  <div className={`text-xs mt-1 ${isSelected ? "text-white" : "text-main-text"}`}>
                    {formatDateNumeric(formatDateToString(item.date))}
                  </div>
                  <div className={`text-xs hidden xs:block ${isSelected ? "text-white" : "text-main-text"}`}>
              ...
              {availableTimeSlots.map((slot) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="hidden xl:block">{formatted.desktopLabel}</span>
                  </button>
                );
              })}
