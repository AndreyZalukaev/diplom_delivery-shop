...
/** Карта с магазинами (Яндекс.Карты) */
const Maps = () => {
  const [currentLocation, setCurrentLocation] = useState("archangelsk");
  const currentLocationData = locations[currentLocation];

  return (
    <YMaps query={{ lang: "ru_RU", apikey: "1ac61b8a-843f-454f-abd8-a651a3c60f00", load: "package.full" }}>
      <section>
        <div className="flex flex-col justify-center xl:max-w-[1208px] text-[#414141]">
          <h2 className="mb-4 md:mb-8 xl:mb-10 text-2xl xl:text-4xl text-left font-bold">Наши магазины</h2>
              ...
              return (
                <button key={key} onClick={() => setCurrentLocation(key)}
                  className={`p-2 text-xs justify-center items-center active:shadow-(--shadow-button-active) border-none rounded cursor-pointer transition-colors duration-300 ${
                    isActive ? "bg-(--color-primary) text-white hover: shadow-(--shadow-button-default)" : "bg-[#f3f2f1] hover:shadow-(--shadow-button-secondary)"
                  }`}>
                  {locations[key].name}
                </button>
              );
          ...
          <Map options={{ suppressMapOpenBlock: true }}
            ...
            {locations[currentLocation].shops.map((shop) => (
              <Placemark key={shop.id} geometry={shop.coordinates}
